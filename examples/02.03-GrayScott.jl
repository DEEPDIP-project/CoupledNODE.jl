...
full_CNODE = NeuralODE(f_CNODE, trange_burn, Tsit5(), adaptive=false, dt=dt_burn, saveat=saveat_burn)
...
u_exact = reshape(untrained_CNODE_solution[1:grid.Nu, :, :]    , grid.nux, grid.nuy, size(untrained_CNODE_solution,2), :)
v_exact = reshape(untrained_CNODE_solution[grid.Nu+1:end, :, :], grid.nvx, grid.nvy, size(untrained_CNODE_solution,2), :);

# Plot the solution as an animation
anim = Animation()
fig = plot(layout = (1, 2), size = (600, 300))
@gif for i in 1:2:size(u_exact, 4)
    p1 = heatmap(u_exact[:,:,1,i], axis=false, bar=false, aspect_ratio=1, color=:reds , title="u(x,y)")
    p2 = heatmap(v_exact[:,:,1,i], axis=false, bar=false, aspect_ratio=1, color=:blues, title="v(x,y)")
...
uv0_dns = vcat(reshape(u0_dns, dns_grid.nux*dns_grid.nuy,1),reshape(v0_dns, dns_grid.nvx*dns_grid.nvy,1))
...
# burnout run
dns_CNODE = NeuralODE(f_dns, trange_burn, Tsit5(), adaptive=false, dt=dt_burn, saveat=saveat_burn)
...
u_dns = reshape(dns_solution[1:dns_grid.Nu, :, :]    , dns_grid.nux, dns_grid.nuy, size(dns_solution,2), :)
v_dns = reshape(dns_solution[dns_grid.Nu+1:end, :, :], dns_grid.nvx, dns_grid.nvy, size(dns_solution,2), :);
...
@gif for i in 1:2:size(u_exact, 4)
    p1 = heatmap(u_exact[:,:,1,i], axis=false, bar=false, aspect_ratio=1, color=:reds , title="u(x,y)")
    p2 = heatmap(v_exact[:,:,1,i], axis=false, bar=false, aspect_ratio=1, color=:blues, title="v(x,y)")
    p3 = heatmap(u_dns[:,:,1,i], axis=false, bar=false, aspect_ratio=1, color=:reds , title="u(x,y)")
    p4 = heatmap(v_dns[:,:,1,i], axis=false, bar=false, aspect_ratio=1, color=:blues, title="v(x,y)")
    time = round(i*saveat, digits=0)
...
uv0_les = vcat(reshape(u0_les, les_grid.nux*les_grid.nuy,1),reshape(v0_les, les_grid.nvx*les_grid.nvy,1))

# Compare the initial conditions
p1= heatmap(u_initial, axis=false, cbar=false, aspect_ratio=1, title="u exact", color=:reds)
p2= heatmap(v_initial, axis=false, cbar=false, aspect_ratio=1, title="v exact", color=:blues)
p3= heatmap(u0_dns, axis=false, cbar=false, aspect_ratio=1, title="u_0 DNS", color=:reds)
p4= heatmap(v0_dns, axis=false, cbar=false, aspect_ratio=1, title="v_0 DNS", color=:blues)
p5= heatmap(u0_les, axis=false, cbar=false, aspect_ratio=1, title="u_0 LES", color=:reds)
p6= heatmap(v0_les, axis=false, cbar=false, aspect_ratio=1, title="v_0 LES", color=:blues)
...
# burnout run
les_CNODE = NeuralODE(f_les, trange_burn, Tsit5(), adaptive=false, dt=dt_burn, saveat=saveat_burn)
...
# And we unpack the solution to get the two species from
u_les = reshape(les_solution[1:les_grid.Nu, :, :]    , les_grid.nux, les_grid.nuy, size(les_solution,2), :)
v_les = reshape(les_solution[les_grid.Nu+1:end, :, :], les_grid.nvx, les_grid.nvy, size(les_solution,2), :);

anim = Animation()
fig = plot(layout = (3, 2), size = (600, 900))
@gif for i in 1:2:size(u_exact, 4)
    p1 = heatmap(u_exact[:,:,1,i], axis=false, cbar=false, aspect_ratio=1, color=:reds , title="u exact")
    p2 = heatmap(v_exact[:,:,1,i], axis=false, cbar=false, aspect_ratio=1, color=:blues, title="v exact")
    p3 = heatmap(u_dns[:,:,1,i], axis=false, cbar=false, aspect_ratio=1, color=:reds , title="u DNS")
    p4 = heatmap(v_dns[:,:,1,i], axis=false, cbar=false, aspect_ratio=1, color=:blues, title="v DNS")
    p5 = heatmap(u_les[:,:,1,i], axis=false, cbar=false, aspect_ratio=1, color=:reds , title="u LES")
    p6 = heatmap(v_les[:,:,1,i], axis=false, cbar=false, aspect_ratio=1, color=:blues, title="v LES")
    time = round(i*saveat, digits=0)
