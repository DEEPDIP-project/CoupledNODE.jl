...
function create_dumb_nn(grid)
    ...
    return Chain(
        uv -> reshape(uv, grid.nux*grid.nuy*2, :),
        Dense(ninput, noutput, leakyrelu, init_weight=Lux.zeros32, init_bias=Lux.zeros32),
...
function create_cnn(grid)
    ...
    return Chain(
        ...
        Chain(
            ...
            Conv((2*r_cnn[i]+1,2*r_cnn[i]+1), ch_cnn[i]=>ch_cnn[i+1], σ[i]; pad=0, stride=stride[i]),
            MaxPool((pool_k[i],pool_k[i])),
