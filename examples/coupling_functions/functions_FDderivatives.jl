...
#function second_derivatives(u, Δx, Δy)
#    # Compute second derivative with respect to x
#    d2u_dx2 = vcat(add_dim_1(u[2, :, :] - 2 * u[1, :, :] + u[end, :, :]) ,
#                  (u[3:end, :, :] - 2 * u[2:(end - 1), :, :] + u[1:(end - 2), :, :]),
#                  add_dim_1(u[1, :, :] - 2 * u[end, :, :] + u[end - 1, :, :]),
#                  ) / (Δx^2)
#
#    ## Compute second derivative with respect to y
#    d2u_dy2 = hcat(add_dim_2(u[:, 2, :] - 2 * u[:, 1, :] + u[:, end, :]),
#                  (u[:, 3:end, :] - 2 * u[:, 2:(end - 1), :] + u[:, 1:(end - 2), :]),
#                  add_dim_2(u[:, 1, :] - 2 * u[:, end, :] + u[:, end - 1, :]),
#                  ) / (Δy^2)
#
#    return d2u_dx2, d2u_dy2
#end
#
#function Laplacian(u, Δx, Δy)
#    return sum(second_derivatives(u, Δx, Δy))
#end

#function Laplacian(u, Δx, Δy)
#    return vcat(add_dim_1(u[2, :, :] - 2 * u[1, :, :] + u[end, :, :]) ,
#                  (u[3:end, :, :] - 2 * u[2:(end - 1), :, :] + u[1:(end - 2), :, :]),
#                  add_dim_1(u[1, :, :] - 2 * u[end, :, :] + u[end - 1, :, :]),
#                  ) / (Δx^2) 
#                  +
#                  hcat(add_dim_2(u[:, 2, :] - 2 * u[:, 1, :] + u[:, end, :]),
#                       (u[:, 3:end, :] - 2 * u[:, 2:(end - 1), :] + u[:, 1:(end - 2), :]),
#                       add_dim_2(u[:, 1, :] - 2 * u[:, end, :] + u[:, end - 1, :]),
#                  ) / (Δy^2)
#
