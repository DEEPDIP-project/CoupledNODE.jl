...
saveat_shock = 0.01f0
dns = create_f_CNODE((F_dns,), (grid_u_dns,), trange_burn, solver_algo; adaptive = false, dt = dt_dns, saveat = saveat_shock, is_closed = false);
les = create_f_CNODE((F_les,), (grid_u_les,), trange_burn, solver_algo; adaptive = false, dt = dt_les, saveat = saveat_shock, is_closed = false);
...
```julia
f_CNODE = create_f_CNODE((F_les,), (grid_u_les,), trange_burn, solver_algo, (NN_u,); adaptive = false, dt = dt_les, saveat = saveat_shock, is_closed = true);
...
f_CNODE_pos = create_f_CNODE((F_les,), (grid_u_les,), t_train_range, solver_algo, (NN_u_pos,); adaptive = false, dt = dt_train, saveat = saveat_train, is_closed = true);
...
```julia
error_posteriori = sum(abs, f_CNODE_pos.model(all_u_les_flat, θ_pos, st_pos)[1] - target_F_flat) /
                   sum(abs, target_F_flat);
