...
# $v$ to the grid of $u$ prior to the force calculation and to transform the results of $v$ back to the coarser grid. This is specific to out problem, but `create_f_CNODE` allows us to define operations that have to take place prior and after the force calculation.
...
using Images
les_CNODE = create_f_CNODE(
    (F_u, G_v), (grid_GS_u, coarse_grid), trange, Tsit5(), pre_force = upscaler_v,
    post_force = downscaler_v, adaptive = false, dt = dt, saveat = saveat);
θ, st = Lux.setup(rng, les_CNODE.model);
...
# We compute from the simulation data the target that we would like to fit. In the example $u$ will be unchanged, while $v$ will be rescaled to the coarse grid. The labels generated are going to be used to calculate the loss during training.  
...
# We define the model to train 
training_CNODE = create_f_CNODE(
    (F_u, G_v), (grid_GS_u, coarse_grid), t_train_range, Tsit5(),
    (NN_u, NN_v), adaptive = false, dt = dt_train, saveat = saveat_train,
    pre_force = upscaler_v, post_force = downscaler_v, is_closed = true);
## Get the model parameters
θ, st = Lux.setup(rng, training_CNODE.model);
...
trained_CNODE = create_f_CNODE(
    (F_u, G_v), (grid_GS_u, coarse_grid), trange, Tsit5(), (NN_u, NN_v),
    adaptive = false, dt = dt_train, saveat = saveat_train,
    pre_force = upscaler_v, post_force = downscaler_v, is_closed = true);
