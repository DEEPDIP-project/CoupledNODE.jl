...
# In this example, we will learn how to do a posteriori fitting, using a multishooting approach.
...
# (1) maximize on the data
# (2) to prevent unstabilities while training
...
# Look at Example 02.01 for more details.
# The difference is that in here we define different architectures for $NN_u$ and $NN_v$, in order to make the training easier.
...
# ### A posteriori **loss function**
# In a posteriori fitting, we rely on a differentiable solver to propagate the gradient through the solution of the NODE. In this case, we use the *multishooting a posteriori* fitting (MulDtO), where we tell `Zygote` to compare `nintervals` of length `nunroll` to get the gradient. Notice that this method is differentiating through the solution of the NODE!
nunroll = 10   
nintervals = 5
nsamples = 1
# We then need to define an auxiliary NODE that will be used for training, because we want to control the timestep and the total lenght of the solutions that we have to compute at each iteration.
# In particular, we can use smaller time steps for the training 
dt_train = 0.001
# but we have to sample at the same rate as the data
saveat_train = saveat
# also it is important to solve for only the time intervall thas is needed at each training step (corresponding to nunroll steps)
t_train_range = (0.0f0, saveat_train*(nunroll+0))
training_CNODE = NeuralODE(f_closed_CNODE, t_train_range, Tsit5(), adaptive=false, dt=dt_train, saveat=saveat_train);
# I will also define a secondary auxiliary CNODE that will be used if the previous one is unstable
