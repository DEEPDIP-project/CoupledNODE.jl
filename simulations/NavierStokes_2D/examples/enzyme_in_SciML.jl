...
function lop(
        θ::Vector{Float64}, u0::Vector{Float64}, tspan::Vector{Float64}, t::Vector{Float64})
    myprob = ODEProblem{true}(dudt, u0, tspan, θ)
...
function lip(l::Vector{Float64}, θ::Vector{Float64}, u0::Vector{Float64},
...
Enzyme.autodiff(Enzyme.Reverse, lop, Active, DuplicatedNoNeed(p_nn, dp),
...
Enzyme.autodiff(Enzyme.Reverse, lip, Duplicated(l, dl), Duplicated(p_nn, dp),
    DuplicatedNoNeed(u0, du0), Const(tspan), Const(t))
...
function my_G(G, θ::Vector{Float64}, u0_tspan_t_dp_du0::Vector{Vector{Float64}})
    ...
    Enzyme.autodiff(Enzyme.Reverse, lip, Duplicated([0.0], [1.0]), Duplicated(θ, G),
        DuplicatedNoNeed(u0, du0), Const(tspan), Const(t))
    nothing
end
...
optf = Optimization.OptimizationFunction(
    (p, _) -> loss_n_ode(p), grad = (G, p, u) -> my_G(G, p, u))
...
p4 = Plots.bar(
    ["Zygote", "Enzyme"], [result_z.objective, result_e.objective], xlabel = "Method",
