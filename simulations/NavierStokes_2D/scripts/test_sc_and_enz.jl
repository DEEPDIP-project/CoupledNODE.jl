...
# [!] CONCLUSION:
# Enzyme is faster than Zygote for both SimpleChain and Lux
# Lux is faster but it uses more memory than SimpleChain
# SimpleChain can NOT run on GPU 
...
p_nn_s = Array(p_nn)
# use the same parameters as Lux
...
function lip_sc(l::Vector{Float64},θ::Vector{Float32}, u0::Vector{Float64}, tspan::Vector{Float64}, t::Vector{Float64})
    myprob = ODEProblem{true}(dudt_sc, u0, tspan, θ)
    pred = Array(solve(myprob, RK4(), u0 = u0, p = θ, saveat=t, verbose=false))
...
function G_s!(G, θ::Vector{Float32}, u0_tspan_t_dp_du0::Vector{Vector{Float64}}) 
    ...
    Enzyme.make_zero!(G)
    Enzyme.autodiff(Enzyme.Reverse, lip_sc, Duplicated([0.0], [1.0]), Duplicated(θ, G), DuplicatedNoNeed(u0, du0) , Const(tspan), Const(t))
    nothing
end
G_s!(copy(dp),p_nn_s, [u0, tspan, t, du0])

optf = Optimization.OptimizationFunction((p,_)->loss_sc(p), grad=(G,p,u)->G_s!(G,p,u))
