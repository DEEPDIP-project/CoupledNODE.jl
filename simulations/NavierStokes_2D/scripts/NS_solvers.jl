...
import Plots
p1 = Plots.bar(
    ["INS", "SciML", "CNODE"], [time_ins, time_ode, time_node], xlabel = "Method",
...
p4 = Plots.heatmap(title = "\$u_{INS} - u_{ODE}\$", state.u[1] - sol_ode.u[end][:, :, 1], ticks = false);
p5 = Plots.heatmap(title = "\$u_{INS}-u_{CNODE}\$", state.u[1] - sol_node.u[end][:, :, 1], ticks = false);
p6 = Plots.heatmap(
    title = "\$u_{CNODE}-u_{ODE}\$", sol_node.u[end][:, :, 1] - sol_ode.u[end][:, :, 1], ticks = false);
...
p1 = Plots.heatmap(title = "\$\\omega\$ SciML ODE", vode, color = :viridis, ticks = false, clims = vor_lims);
p2 = Plots.heatmap(title = "\$\\omega\$ in SciML CNODE", vnode, color = :viridis, ticks = false, clims = vor_lims);
p3 = Plots.heatmap(title = "vorticity \$(\\omega)\$ in INS", vins, color = :viridis, ticks = false, clims = vor_lims);
p4 = Plots.heatmap(title = "\$\\omega_{INS}-\\omega_{ODE}\$", vins - vode, clim=diff_lims, ticks = false);
p5 = Plots.heatmap(title = "\$\\omega_{INS}-\\omega_{CNODE}\$", vins - vnode, clim=diff_lims, ticks = false);
p6 = Plots.heatmap(title = "\$\\omega_{CNODE}-\\omega_{ODE}\$", vode - vnode, ticks = false, clim=(0, 0.2));
...
for (idx, (t, u)) in enumerate(zip(sol_ode.t, sol_ode.u))
    ...
    fig = Plots.heatmap(ω'; xlabel = "x", ylabel = "y", title, clims = vor_lims, color = :viridis)
    frame(anim, fig)
end
